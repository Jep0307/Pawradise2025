...
signinForm.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    if (error) {
      ...
      const response = await fetch("http://localhost/SIA02/Pawradise2025/admin/components/checktherole.php", {
        method: "POST",
        headers: {
      ...
      if (result.status === "success") {
        ...
        if (role === "admin") {
          ...
          formMessage.classList.add("success");
          setTimeout(() => {
            window.location.href = "http://localhost/SIA02/Pawradise2025/admin/tabs/dashboard.php";
          }, 2000);
        } else if (role === "staff") {
          formMessage.textContent = "Sign-in successful! Redirecting...";
          formMessage.classList.add("success");
          setTimeout(() => {
            window.location.href = "http://localhost/SIA02/Pawradise2025/staff/pages/petsmanagement.html";
          }, 2000);
        } else {
          formMessage.textContent = "Access denied: unauthorized role.";
          formMessage.classList.add("error");
        }
      } else {
